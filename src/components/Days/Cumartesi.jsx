...
const Cumartesi = () => {
  ...
  const studyCheckFunc = () => {
    const thisDayStudy = Number(localStorage.getItem("cumartesi"));
    if (thisDayStudy !== -1) {
  ...
  const selectStudiedFunc = (studiedTime) => {
    localStorage.setItem("cumartesi", studiedTime);
  ...
  if (selectedTarget === false) {
    return (
      <div className="day">
        <p className="day-title">Cumartesi Hedef</p>
    ...
    return (
      ...
      >
        <p className="day-title">Cumartesi Yapılan</p>
        {studyCheck ? (
