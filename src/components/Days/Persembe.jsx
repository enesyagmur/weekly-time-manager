...
const Persembe = () => {
  ...
  const studyCheckFunc = () => {
    const thisDayStudy = Number(localStorage.getItem("persembe"));
    if (thisDayStudy !== -1) {
  ...
  const selectStudiedFunc = (studiedTime) => {
    localStorage.setItem("persembe", studiedTime);
  ...
  if (selectedTarget === false) {
    return (
      <div className="day">
        <p className="day-title">Perşembe Hedef</p>
    ...
    return (
      ...
      >
        <p className="day-title">Perşembe Yapılan</p>
        {studyCheck ? (
