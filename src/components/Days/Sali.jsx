...
const Sali = () => {
  ...
  const studyCheckFunc = () => {
    const thisDayStudy = Number(localStorage.getItem("sali"));
    if (thisDayStudy !== -1) {
  ...
  const selectStudiedFunc = (studiedTime) => {
    localStorage.setItem("sali", studiedTime);
  ...
  if (selectedTarget === false) {
    return (
      <div className="day">
        <p className="day-title">Salı Hedef</p>
    ...
    return (
      ...
      >
        <p className="day-title">Salı Yapılan</p>
        {studyCheck ? (
