...
const Cuma = () => {
  ...
  const studyCheckFunc = () => {
    const thisDayStudy = Number(localStorage.getItem("cuma"));
    if (thisDayStudy !== -1) {
  ...
  const selectStudiedFunc = (studiedTime) => {
    localStorage.setItem("cuma", studiedTime);
  ...
  if (selectedTarget === false) {
    return (
      <div className="day">
        <p className="day-title">Cuma Hedef</p>
    ...
    return (
      ...
      >
        <p className="day-title">Cuma Yapılan</p>
        {studyCheck ? (
