...
const Carsamba = () => {
  ...
  const studyCheckFunc = () => {
    const thisDayStudy = Number(localStorage.getItem("carsamba"));
    if (thisDayStudy !== -1) {
  ...
  const selectStudiedFunc = (studiedTime) => {
    localStorage.setItem("carsamba", studiedTime);
  ...
  if (selectedTarget === false) {
    return (
      <div className="day">
        <p className="day-title">Çarşamba Hedef</p>
    ...
    return (
      ...
      >
        <p className="day-title">Çarşamba Yapılan</p>
        {studyCheck ? (
