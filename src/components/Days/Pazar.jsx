...
const Pazar = () => {
  ...
  const studyCheckFunc = () => {
    const thisDayStudy = Number(localStorage.getItem("pazar"));
    if (thisDayStudy !== -1) {
  ...
  const selectStudiedFunc = (studiedTime) => {
    localStorage.setItem("pazar", studiedTime);
  ...
  if (selectedTarget === false) {
    return (
      <div className="day">
        <p className="day-title">Pazar Hedef</p>
    ...
    return (
      ...
      >
        <p className="day-title">Pazar Yapılan</p>
        {studyCheck ? (
